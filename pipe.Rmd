...
Dificilmente um programador de R passa muito tempo sem ouvir falar do operador
pipe (`%>%`). Saber o que ele faz e significa, no entando, é algo mais complexo.
...
portugês o termo é traduzido preferencialmente como "encadeamento", mas no
...
agumento da função à sua direita. Isso não impede que as funções utilizadas em
...
_debugabilidade_^[Ainda não encontrei um termo melhor em portugês para esse
...
que não quer entender o significado do código a seguir:
